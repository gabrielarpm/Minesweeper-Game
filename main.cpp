...
//	None
void initField(int num_of_mine) {
    //This code initializes every cell to UNKNOWN, which is the initial state of the cell.
    // We are going through each variable (i and j, which are the row and column indexes) and changing it to UNKNOWN
    // The row starts at 0 and goes up to 7. The column starts at 0 and goes up to 4.
    // SetCell is sending the variables i and j and the UNKNOWN value we want to change it to, which is the dash.
    // SetMine is sending the variable num_of_mine, which tell use the number of mines there are in the game.
    for(int row=0; row<8; row++){
        for(int column=0; column<5; column++){
            setCell(row, column, UNKNOWN);
        }
    }
    setMine(num_of_mine);
}
...
bool isMine(int i, int j) {
    // In this code, if it's an UNFLAGGED_MINE or FLAGGED_MINE, you return true, and if it's not you return false.
...
//	None
void reveal(int i,int j) {
    //If we do the reveal on a cell, it’s going to check the surrounding cells to see if there are mines.
    //If there are mines, then the cell is going to change it's countMines value, so each time it sees a cell, it will increment it.
    //At the end it will set the revealed cell’s value to that counter.
    //The if statements are going through 8 different cases (or cells) of the revealed cell and checking the surrounding row and column indexes using increments and decrements on i and j.
    //The conditions are checking if the surrounding cells are out of bound.
    int countMines = 0;
    ...
    if(i-1>=0 && j+1<=4 && isMine(i-1,j+1)) {
        countMines++;
    }
    if(j-1>=0 && isMine(i,j-1)) {
        countMines++;
    }
    if(j+1<=4 && isMine(i,j+1)) {
        countMines++;
    }
    if(i+1<=7 && j-1>=0 && isMine(i+1,j-1)) {
        countMines++;
    }
    if(i+1<=7 && isMine(i+1,j)) {
        countMines++;
    }
    if(i+1<=7 && j+1<=4 && isMine(i+1,j+1)) {
        countMines++;
    }
    setCell(i,j, countMines);
}

void undo(int row, int column){
    //This code changes the cell value back to its original value and the number of mines left will also increase.
    if (getCell(row, column)==INCORRECT_FLAGGED_MINE) {
        setCell(row, column, UNKNOWN);
        num_incorrect_flagged_mine --;
    }
    if (getCell(row, column)==FLAGGED_MINE) {
        setCell(row, column, UNFLAGGED_MINE);
        num_correct_flagged_mine --;
    }
}
void flag(int row, int column){
    //This code changes the cell value to an incorrect_flagged_mine or a flagged_mine. The number of mines left will decrease.
    if (getCell(row, column)==UNKNOWN) {
        setCell(row, column, INCORRECT_FLAGGED_MINE);
        num_incorrect_flagged_mine ++;
    }
    if (getCell(row, column)==UNFLAGGED_MINE) {
        setCell(row, column, FLAGGED_MINE);
        num_correct_flagged_mine ++;
    }
...
//	None
void executeCmd(string cmd) {
    //This code verifies that the value of the global explode variable, num_correct_flagged_mine, and num_incorrect_flagged_mine are appropriately set by executeCmd.
    // a,i,j are together as a string and this code separates the string.
    //The first position has a 0 (a), the second position has a 1 (i) and the third position has a 2(j).
    //int i and int j are returning a character, so they are being casted into integers
    //Using the ASCII table the code is subtracting 48 from i and j so that any of the chars on that table just subtract it to the “dec” and then it will give you 48
    char a = cmd.at (0);
    int i = (int) cmd.at (1)-48;
    int j = (int) cmd.at (2)-48;
    if(a=='u'){
        undo(i,j);
    }
    if(a=='f'){
